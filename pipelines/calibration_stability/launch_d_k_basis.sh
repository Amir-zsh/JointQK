...
#
# Usage:
#   bash pipelines/calibration_stability/launch_d_k_basis.sh --gpus 6,7
#   tail -f logs/phase1d_k_basis_stability.log
...
LOG="${REPO_ROOT}/logs/phase1d_k_basis_stability.log"
...
for shard_id in "${!GPU_ARR[@]}"; do
    ...
    cmd="CUDA_VISIBLE_DEVICES=${gpu} ${REPO_ROOT}/.venv/bin/python ${REPO_ROOT}/pipelines/calibration_stability/run_d_k_basis.py \
        --device cuda \
        --bundle ${BUNDLE} \
        --output-dir ${OUT} \
...
"${REPO_ROOT}/.venv/bin/python" "${REPO_ROOT}/pipelines/calibration_stability/aggregate_d_k_basis.py" \
    --rows "${rows_files[@]}" \
