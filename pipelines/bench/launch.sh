...
if [[ ! -f "$CCA" ]]; then
    ...
    echo "Build it with: .venv/bin/python pipelines/scripts/build_calibration_artifacts_from_pool.py --run-id longbench_compact8_qkv_${MODEL_TAG} --output-suffix ${CCA_SUFFIX}" >&2
    exit 1
fi
...
LOG_DIR="${REPO_ROOT}/logs/phase7_v7_${MODEL_TAG}"
...
${VENV_PYTHON} pipelines/bench/worker.py \
    --model "$MODEL" \
    --commands-file "$CMDS" \
    --log-dir "$LOG_DIR" \
    --gpus "$GPUS" \
