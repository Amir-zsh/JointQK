...
Usage:
  python -u pipelines/throughput/run_throughput.py \
      --config pipelines/throughput/configs/qwen3_8b.json --gpus 0,1,2
"""
...
from pipelines.throughput import capacity  # noqa: E402
...
class Server:
    """One serve_oscar.sh instance, port-scoped so concurrent runs cannot
    ...
    def _kill(self) -> None:
        sh(["docker", "exec", CONTAINER, "bash", "-lc",
            f"pkill -9 -f '[l]aunch_server.*--port {self.port}'"])
    ...
    def start(self, mem_frac: float) -> None:
        ...
        env = {
            ...
            # "auto" omits --max-total-tokens so the pool follows mem-fraction;
            # serve_oscar.sh otherwise pins 140000, which would cap every arm at
            # the same tiny pool and erase the entire capacity comparison.
            "MAX_TOKENS": "auto",
            "RADIX_CACHE": "1",
            "MAX_REQS": str(self.max_reqs),
            "CUDA_GRAPH_BS": str(self.graph_bs),
            "PREFILL_BACKEND": s.get("prefill_backend", "fa3"),
            # Chunk size drives how much of a long prompt the quant arms can
            # retain: the mixed-KV prefix-reuse fix commits only up to the last
            # COMPLETE prefill chunk minus the recent window, so the cached
            # fraction is floor((L-1)/C)*C - RECENT. Smaller C -> higher
            # coverage. At C=8192 a 30k prompt caches 81%; at 4096, 94.7%.
            "SERVE_EXTRA": ("--enable-cache-report --chunked-prefill-size "
                            f"{s.get('chunked_prefill_size', 4096)} "
                            + s.get("extra", "")).strip(),
            "ROT_DIR": str(_abs(self.cfg["synthetic_root"]) / "rotations"),
            "TP": str(self.tp),
        }
        if self.arm["label"] != "bf16":
        ...
        env.update(self.arm.get("env", {}))

        eargs = []
        for k, v in env.items():
            eargs += ["-e", f"{k}={v}"]
        cmd = (f"cd {REPO} && bash pipelines/oscar_e2e/serve_oscar.sh "
               f"--gpu {','.join(str(g) for g in self.gpus)} --port {self.port} "
               f"--ctx {s['ctx']} --mem-frac {mem_frac} "
...
def run_arm(arm: dict, cfg: dict, geom: dict, gpus: list[int], port: int,
    ...
    predicted_cell = capacity.cell_size(label if label == "bf16" else "int2", geom, tp)
    ...
    while not fits and b_needed > 4:
        ...
        fits, budget = capacity.fits_budget(geom, label, pool_cal, b_needed,
                                            mem_frac, card, tp)
    log(f"{label}: pool={pool_cal:,} cell={predicted_cell}B "
        f"graph_bs={b_needed} hp={budget.get('hp_gib')}GiB mem_frac={mem_frac}")
    ...
    (out_dir / label / "provenance.json").write_text(json.dumps({
        ...
        "cuda_graph_max_bs": b_needed, "flags": arm.get("flags", ""),
...
def main() -> int:
    ...
    out_dir = REPO / "artifacts/throughput" / cfg["study"]
