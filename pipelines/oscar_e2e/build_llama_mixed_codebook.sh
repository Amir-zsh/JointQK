#!/bin/bash
# Mixed-domain Llama codebook retrain (report12 §4 follow-up): same recipe as
# build_llama31_8b.sh stage C+V but calibrated on the five-domain corpus from
# build_mixed_corpus.py instead of GPQA-only. Rotations are reused unchanged —
# only the codebook moves, so any NIAH delta is attributable to calibration
# domain alone.
#
# Usage: build_llama_mixed_codebook.sh <gpuA> <gpuB>
set -u
ROOT="${ROOT:-$(cd "$(dirname "$0")/../.." && pwd)}"
cd "$ROOT"
GPU_A="${1:?gpuA}"; GPU_B="${2:?gpuB}"
MODEL="meta-llama/Llama-3.1-8B-Instruct"
OUT="$ROOT/artifacts/oscar_llama31_8b"
ROT="$OUT/rotations"
CORPUS="$OUT/mixed_corpus.jsonl"
BAS="$OUT/basis_moments_mixed"; POOL="$OUT/query_stats_mixed"
CBRAW="$OUT/vqa_llama31_8b_G4_strat_flat_ptn_mixed64k.pt"
CBFP8="$OUT/vqa_llama31_8b_G4_strat_flat_ptn_mixed64k_fp8.pt"
PY="$ROOT/.venv/bin/python"
LOG="$ROOT/logs/build_llama_mixed.log"; HB="$ROOT/logs/build_llama_mixed.heartbeat"
mkdir -p "$OUT" logs
log(){ echo "[$(date '+%F %T')] $*" >> "$LOG"; touch "$HB"; }
space_guard(){ local free=$(df --output=avail -BG /vault | tail -1 | tr -dc 0-9)
  [ "$free" -ge "${1:-15}" ] || { log "DISK GUARD: only ${free}G free (<${1}G) — abort"; exit 3; }; }
log "=== mixed codebook build start gpus=$GPU_A,$GPU_B"
[ -f "$CORPUS" ] || { log "corpus missing: $CORPUS"; exit 1; }
[ -f "$ROT/k_rotation_qqt_r_h_pbr.pt" ] || { log "rotations missing"; exit 1; }
space_guard 25

# --- C1: mixed 64K-concat capture (both GPUs, device_map=auto shards weights)
log "C1 mixed concat-64k capture"
if [ ! -f "$BAS/basis_moments.pt" ]; then
  mkdir -p "$BAS"
  CUDA_VISIBLE_DEVICES=$GPU_A,$GPU_B PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True $PY -u \
    pipelines/oscar_e2e/capture_mixed_concat.py --model "$MODEL" --corpus "${CORPUS#$ROOT/}" \
    --target-ctx 65536 --n-sequences 8 --pool-stride 4 \
    --out-basis "$BAS/basis_moments.pt" --out-pool "$POOL" >> "$LOG" 2>&1
fi
[ -f "$BAS/basis_moments.pt" ] || { log "C1 FAILED"; exit 1; }
NEX=$(ls "$POOL/examples" 2>/dev/null | wc -l); log "C1 done pool examples=$NEX"
space_guard 15

# --- C2/C3: train + fp8 (identical flags to the GPQA-only build)
log "C2 codebook train (stratified flat ptn bpc2)"
if [ ! -f "$CBRAW" ]; then
  IDX=$(seq 0 $((NEX-1)) | paste -sd' ')
  CUDA_VISIBLE_DEVICES=$GPU_B $PY -u third_party/samuel_vq/train_group_vq_alloc.py \
    --basis-moments "$BAS/basis_moments.pt" --data-root "$POOL" --code-idx $IDX \
    --grouping stratified --allocation flat --bpc 2 --pertoken-norm \
    --out "$CBRAW" >> "$LOG" 2>&1
fi
[ -f "$CBRAW" ] || { log "C2 FAILED"; exit 1; }
$PY third_party/samuel_vq/make_fp8.py --in "$CBRAW" --out "$CBFP8" --fmt e5m2 >> "$LOG" 2>&1
[ -f "$CBFP8" ] || { log "C3 fp8 FAILED"; exit 1; }
log "C DONE -> $CBFP8; deleting pool (disk)"
rm -rf "$POOL"

# --- V: gates + vq2 smoke on the new bundle
log "V1 gates on mixed bundle"
PYTHONPATH="$ROOT/vendor/OSCAR-vq/sglang-research/python" CUDA_VISIBLE_DEVICES=$GPU_A \
  timeout 600 "$ROOT/.venv-oscar/bin/python" pipelines/oscar_e2e/verify_vq_engine.py \
  --bundle "${CBFP8#$ROOT/}" --v-bundle /nonexistent --layers 0 5 18 31 >> "$LOG" 2>&1
grep -q "ALL GATES PASS" <(tail -5 "$LOG") || { log "V1 FAILED"; exit 1; }
log "V1 PASS"

export ROT_DIR="$ROT"
log "V2 vq2 smoke (mixed codebook)"
nohup bash pipelines/oscar_e2e/serve_oscar.sh --vq2 --model "$MODEL" \
  --vq-codebook "${CBFP8#$ROOT/}" --gpu "$GPU_A" --port 30830 \
  > logs/llama_vq2mix_smoke.log 2>&1 &
SPID=$!
for i in $(seq 1 90); do
  grep -q "The server is fired up and ready to roll" logs/llama_vq2mix_smoke.log && break
  grep -qE "Received sigquit|CUDA out of memory|Not enough memory" logs/llama_vq2mix_smoke.log && break
  sleep 5; touch "$HB"
done
if grep -q "fired up" logs/llama_vq2mix_smoke.log; then
  python3 -c "
import json, urllib.request
p={'text':'The capital of France is','sampling_params':{'temperature':0.0,'max_new_tokens':16}}
r=urllib.request.Request('http://127.0.0.1:30830/generate', json.dumps(p).encode(), {'Content-Type':'application/json'})
print('vq2mix gen:', json.loads(urllib.request.urlopen(r,timeout=180).read())['text'][:80])" >> "$LOG" 2>&1 \
    && log "V2 smoke PASS" || { log "V2 smoke GEN-FAIL"; exit 1; }
else
  log "V2 smoke BOOT-FAIL"; tail -8 logs/llama_vq2mix_smoke.log >> "$LOG"; exit 1
fi
kill $SPID 2>/dev/null; P=$(lsof -t -i :30830 2>/dev/null); [ -n "$P" ] && kill $P; sleep 8

log "=== BUILD_MIXED_DONE codebook=$CBFP8"
echo BUILD_MIXED_DONE >> "$LOG"
