#!/bin/bash
# gpqacc128k: Llama K codebook calibrated on the SAME 198-prompt GPQA corpus
...
#
# Usage: build_llama_128k_codebook.sh <gpuA> <gpuB> <gpuC>   (capture shards
# across all three; 128K prefill captures need the headroom)
set -u
ROOT="${ROOT:-$(cd "$(dirname "$0")/../.." && pwd)}"
cd "$ROOT"
GPU_A="${1:?gpuA}"; GPU_B="${2:?gpuB}"; GPU_C="${3:?gpuC}"
MODEL="meta-llama/Llama-3.1-8B-Instruct"
OUT="$ROOT/artifacts/oscar_llama31_8b"
CORPUS="$OUT/gpqa_only_corpus.jsonl"
BAS="$OUT/basis_moments_128k"; POOL="$OUT/query_stats_128k"
CBRAW="$OUT/vqa_llama31_8b_G4_strat_flat_ptn_gpqacc128k.pt"
CBFP8="$OUT/vqa_llama31_8b_G4_strat_flat_ptn_gpqacc128k_fp8.pt"
PY="$ROOT/.venv/bin/python"
LOG="$ROOT/logs/build_llama_128k.log"; HB="$ROOT/logs/build_llama_128k.heartbeat"
mkdir -p logs
