// fused_attn.cu — FUSED: per (query-tile, gs-head) block decodes compressed K-pages
// into SRAM and runs causal flash attention against them. K never materialized in HBM
// as fp16; only compressed bytes are read. Mixed-rung pages handled via per-page rung id.
...
__global__ void fused_attn_kernel(
    // compressed K, full head, pages in order
    const uint8_t* __restrict__ blobs,
    const long*    __restrict__ page_byte_off,   // (nb,)
    const long*    __restrict__ lane_off,        // (nb, N)
    const int*     __restrict__ k0a,             // (nb, N)
    const int*     __restrict__ k1a,             // (nb, N)
    const int*     __restrict__ ns,              // (nb,)
    const int*     __restrict__ rung_of_page,    // (nb,)
    // stacked per-rung tables
    const int*     __restrict__ nonconst_cat,    // cat over rungs
    const int*     __restrict__ nonconst_base,   // (R,)
    const int*     __restrict__ C_all,           // (R,)
    const unsigned int* __restrict__ cdf_flat,   // cat over rungs
    const int*     __restrict__ cdf_flat_base,   // (R,)
    const int*     __restrict__ cdf_off_all,     // (R, d+1)
    const long*    __restrict__ vals_all,        // (R, d, A_max)
    const int*     __restrict__ cv_all,          // (R, d)
    const unsigned char* __restrict__ nc_mask_all, // (R, d)
    const float*   __restrict__ delta_all,       // (R,)
    // query (one gs-head) and value
...
{
    int pid_m = blockIdx.x;          // query tile
    int pid_h = blockIdx.y;          // which gs-head
    int tid = threadIdx.x;
    ...
    float* s_q    = (float*)(s_pos + P * d);          // BLOCK_M*d floats

    int q0 = pid_m * BLOCK_M;
    // load q' tile into SRAM
    for (int i = tid; i < BLOCK_M * d; i += nthreads) {
        int r = i / d, c = i % d;
        int qg = q0 + r;
        s_q[i] = (qg < T) ? Q[((long)pid_h * T + qg) * d + c] : 0.0f;
    }
    __syncthreads();

    // per-query-row online-softmax state (thread = query row)
    int qrow = q0 + tid;
    ...
    int last_page = ((q0 + BLOCK_M - 1) < T ? (q0 + BLOCK_M - 1) : (T - 1)) / P;
    ...
    for (int pg = 0; pg <= last_page; ++pg) {
        ...
        // attention: each query-row thread sweeps this page's keys
        if (tid < BLOCK_M && qrow < T) {
            int base_n = pg * P;
            for (int kk = 0; kk < P; ++kk) {
                int n = base_n + kk;
                if (n >= T || n >= base_n + nrows) break;
                if (n > qrow) break;                  // causal
                float s = 0.0f;
                for (int c = 0; c < d; ++c) s += s_q[tid * d + c] * s_rhat[kk * d + c];
                s *= sm_scale;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("fused_attn", &fused_attn, "fused decode+causal-attention, compressed K in SRAM");
}