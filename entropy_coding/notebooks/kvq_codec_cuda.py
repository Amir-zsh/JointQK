#!/usr/bin/env python3
"""PageCodecRANSCUDA – GPU-accelerated rANS decode for PageCodecRANS.

Encode is unchanged (CPU, via PageCodecRANS).  Only decode() is overridden:
  - Parses the buffer, groups pages by rung.
  - Calls the CUDA kernel (rans_decode.cu) once per rung-group.
  - Maps symbol positions → quantisation-index values → K_hat.

K_hat is bit-exact with PageCodecRANS.decode() provided the CDF tables match
the encoder's FreqTable.  See _extract_freqs() if they don't.

Drop-in replacement in test_codec_on_data.py:

    from kvq_codec_cuda import build_codecs_from_ladder_rans_cuda, load_ext
    ext = load_ext()          # compiles rans_decode.cu once
    codecs = build_codecs_from_ladder_rans_cuda(
        F, inv, k_mean, ladder, L, Hkv, page_bits, P_tok, dz,
        lanes=N, ext=ext, device="cuda")
    # then pass `codecs` wherever PageCodecRANS codecs were used.
""" 
...
import numpy as np
import torch
from torch.utils.cpp_extension import load as _cu_load
import rans_interleaved as rans

# Import the CPU codec we inherit from.
# Adjust the import path if your file is named differently.
from kvq_codec import _timed, _dz_dequant, _unpack
from kvq_codec_rans import PageCodecRANS   # adjust if needed
from kvq_codec import _timed, _dz_dequant, _unpack, _pack, _np as _np_local
import time

# --------------------------------------------------------------------------- #
# Constants (must match rans_decode.cu)
SCALE_BITS = 14
...
def _extract_freqs(ft) -> np.ndarray:
    ...
    Tries common attribute names in order.  If none match, falls back to
    recomputing from the probability vector stored in the paired _CoordModel.

    If the fallback produces wrong results (decode mismatch), inspect
    rans_interleaved.FreqTable and add the correct attribute name here.
    """
    ...
    # FreqTable doesn't expose integer freqs — signal the caller to fall back.
...
class PageCodecRANSCUDA(PageCodecRANS):
    ...
    def __init__(self, fwd, inv, mu, rungs, page_bits, P_tok, dz,
                 lanes: int = 1, ext=None, enc_ext=None, device: str = "cuda"):        # 1. Let the base class safely parse the raw ladder tuples into real objects first!
        ...
        for ri in range(self.R):
            ...
            for j, m in enumerate(models_r):
                if m.constant:
                    # Constant values are inside the m.vals array/scalar
    ...
    def decode(self, buf: bytes) -> np.ndarray:
        ...
        # Pre-upload vals tables per rung (done once at construction, see __init__)
        # pos_gpu stays on GPU — no .cpu() transfer between kernel and dequant.

        # 3. Group pages by rung and call kernel once per group -----------------
        # pos_out[bi, :n, :] on GPU — symbol positions (index into m.vals)
        # We accumulate results into a single (T, d) idx tensor on GPU.
        idx_gpu = torch.zeros((T, self.d), dtype=torch.int64, device=self.device)
        ...
        with _timed("decode.rangedecode"):
            for ri, pages in rung_groups.items():
                ...
                for pi, (bi, _, n_toks) in enumerate(pages):
                    ...
                    # non-constant coords: gather from vals table
                    # pos_page[:, j] indexes into vals_t[j]
                    # vals_t[j, pos_page[:, j]] — batched with advanced indexing:
                    #   idx_gpu[sl, j] = vals_t[j][pos_page[:, j]]
                    # Use expand + gather for a single kernel call:
                    j_idx = torch.arange(self.d, device=dev)                  # (d,)
                    gathered = vals_t[j_idx.unsqueeze(1),                     # (d, n_toks)
                                      pos_page.t()]
                    # For constant coords, overwrite with const value
        ...
        with _timed("decode.dequant"):
            # Build per-token delta and dz tensors from rung_ids
    ...
    def validate_one_page(self, buf: bytes, page_idx: int = 0) -> bool:
        """Decode a single page with both the CPU and GPU paths and compare K_hat.

        Returns True if they agree to float32 precision.  Useful to confirm that
        _build_rung_cdf() extracted CDFs that match the encoder.
        """
        from kvq_codec import _unpack, _dz_dequant
        import constriction
...
def precompute_all_ladder_data(ladder, n_layers, n_kv, d):
    """
    Scans the entire ladder and precomputes integer frequencies, padding layouts,
    and masks for ALL layers, heads, and rungs simultaneously using fast NumPy vectorization.
    """
    cache = {}
    for ri, (m_scale, delta, model_dict) in enumerate(ladder):
        cache[ri] = {}
        
        # 1. Determine the maximum alphabet size for this rung across the entire grid
        A_max = 1
        ...
        # 2. Allocate structures for all heads and coordinates
        items = []  # Tracks non-constant items for batched quantization: ((l, h), j, m.p)
        for (l, h), models in model_dict.items():
            cache[ri][(l, h)] = {
                'nc_mask': np.zeros(d, dtype=bool),
                'cv': np.zeros(d, dtype=np.int64),
        ...
        # 3. Vectorized Hamilton (Largest-Remainder) Rounding for this entire rung
        ...
        raw = P_batch * TOTAL
        freqs = np.floor(raw).astype(np.int64)
        deficit = TOTAL - freqs.sum(axis=-1, keepdims=True)
        
        fracs = raw - freqs
        fracs[~mask_batch] = -1e9  # Hide padded elements from getting the deficit
        ...
        # 4. Disperse the computed frequencies back to our head-sclices
...
class BatchRANSDecoder:
    ...
    def decode_grid(self, bufs_grid):
        # reset per-codec kernel-event logs
