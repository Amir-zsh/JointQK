...
# ---------------------------------------------------------------------------
# EOB (end-of-block) coding. QPCA orders coords by energy (coord 0 = most), so
# coords quantized to zero cluster at the high-index tail. Instead of coding that
# trailing run of zeros per coord, code a single per-block length L = 1 + index of
# the last nonzero coord (L=0 == all-zero block, the literal "EOB when everything
# is zero" case). Coords >= L are implicitly zero and cost nothing.
#
# This can ONLY beat the independent per-coord coder to the extent the zero pattern
# is CORRELATED across coords beyond the marginals (a token's scale/rank sets how
# many coords stay active). If zeros were independent, the per-coord range coder
# already pays -log2 P(0) and the L symbol is pure overhead. The held-out EOB rate
# below measures whether that correlation is actually there.
# ---------------------------------------------------------------------------
def _eob_lengths(idx):
    """idx: (N,d) int -> L (N,) = 1 + last-nonzero-coord-index, 0 for an all-zero row."""
    d = idx.shape[1]
    nz = idx != 0
    pos = torch.arange(d).unsqueeze(0)
    last = torch.where(nz, pos, torch.full_like(pos, -1)).max(dim=1).values
    return last + 1


def freeze_eob_model(fetch, delta, n_layers, n_kv_heads, d):
    """On CALIB codes: per (l,h) a categorical over the block length L in {0..d}, plus
    per-coord value models fit ONLY on CODED positions (rows with L>j) -- the values
    actually transmitted under EOB, so the model isn't diluted by trailing zeros."""
    model = {}
    for l in range(n_layers):
        for hh in range(n_kv_heads):
            r = fetch(l, hh)
            idx = torch.round(r / delta[l, hh].double().clamp_min(1e-12)).long()
            L = _eob_lengths(idx)
            Lvals, Lcnt = torch.unique(L, return_counts=True)
            Lp = (Lcnt.double() / Lcnt.sum()).clamp_min(1e-12)
            Lp = (Lp / Lp.sum()).cpu().numpy().astype(np.float64)
            per = []
            for j in range(d):
                coded = L > j
                if int(coded.sum()) == 0:
                    per.append((np.array([0], np.int64), np.array([1.0], np.float64)))
                    continue
                vals, counts = torch.unique(idx[coded, j], return_counts=True)
                p = (counts.double() / counts.sum()).clamp_min(1e-12)
                p = (p / p.sum()).cpu().numpy().astype(np.float64)
                per.append((vals.cpu().numpy().astype(np.int64), p))
            model[(l, hh)] = (Lvals.cpu().numpy().astype(np.int64), Lp, per)
    return model


def _coded_bits_with_model(sym, p):
    """bits to range-code an int32 symbol array under categorical p (incl. flush)."""
    import constriction
    enc = constriction.stream.queue.RangeEncoder()
    enc.encode(sym, constriction.stream.model.Categorical(p, perfect=False))
    return len(enc.get_compressed()) * 32


def _snap_to_alphabet(col, vals):
    """map eval symbols onto the (sorted) calib alphabet -> nearest-bin indices."""
...
def coded_bits_eob_eval(fetch_eval, delta, eob_model, n_layers, n_kv_heads, d):
    """Held-out bits/coord under EOB: per block code L with the frozen L-model, then
    code coords 0..L-1 with the frozen coded-position value models; coords >= L free.
    Same denominator (tot_tok * d) as coded_bits_eval, so the two are comparable.
    Layer 0 excluded to match the metrics/baseline."""
    tot_bits, tot_tok = 0, 0
    for l in range(1, n_layers):
        for hh in range(n_kv_heads):
            r = fetch_eval(l, hh)
            idx = torch.round(r / delta[l, hh].double().clamp_min(1e-12)).long()
            L = _eob_lengths(idx).cpu().numpy()
            idx = idx.cpu().numpy()
            tot_tok += idx.shape[0]
            Lvals, Lp, per = eob_model[(l, hh)]
            if Lvals.size > 1:                                  # code the block length
                tot_bits += _coded_bits_with_model(_snap_to_alphabet(L, Lvals), Lp)
            for j in range(d):                                  # code values for j < L
                vals, p = per[j]
                if vals.size <= 1:
                    continue
                coded = L > j
                if not coded.any():
                    continue
                tot_bits += _coded_bits_with_model(_snap_to_alphabet(idx[coded, j], vals), p)
    return tot_bits / max(1, tot_tok * d)


def build_eob_model(delta, fetch_calib, root, calib_idx, b, n_layers, n_kv_heads, d):
    """Fit + cache the EOB model on CALIB. Keyed by (dataset, calib-idx, bits), so it
    is reused across eval splits just like the EC model."""
    MOMENTS_CACHE.mkdir(parents=True, exist_ok=True)
    key = f"{root.name}__eobmodel__idx_" + "_".join(str(i) for i in sorted(calib_idx)) + f"__b{b}"
    cache_path = MOMENTS_CACHE / f"{key}.pt"
    if cache_path.exists():
        c = torch.load(cache_path, map_location="cpu", weights_only=False)
        print(f"  [EOB] b={b}: model CACHE HIT {cache_path.name}")
        return c["model"]
    print(f"  [EOB] b={b}: model cache miss, fitting on calib...")
    model = freeze_eob_model(fetch_calib, delta, n_layers, n_kv_heads, d)
    torch.save({"model": model, "calib_idx": sorted(calib_idx), "b": b}, cache_path)
    print(f"    cached EOB model -> {cache_path.name}")
    return model


def build_qpca_ec(qpca_cen, qpca_unc, k_mean, b, n_layers, n_kv_heads,
                  fetch_calib, root, calib_idx, verbose=False):
    """delta + entropy model from CALIB codes. Cached on disk keyed by
    (dataset, calib-idx, bits) -- independent of eval-idx -- so sweeping eval
    splits skips the calib raw-example walk. Returns (comps, delta, model)."""
    F = qpca_cen["forward"]
    d = F.shape[-1]
    MOMENTS_CACHE.mkdir(parents=True, exist_ok=True)
    key = f"{root.name}__ecmodel__idx_" + "_".join(str(i) for i in sorted(calib_idx)) + f"__b{b}"
    cache_path = MOMENTS_CACHE / f"{key}.pt"
    if cache_path.exists():
        c = torch.load(cache_path, map_location="cpu", weights_only=False)
        ...
        torch.save({"delta": delta, "model": model, "calib_idx": sorted(calib_idx), "b": b},
                   cache_path)
...
METHODS = ["TurboQuant", "PCA", "JointQK", "QPCA", "QPCA-EC", "QPCA-EOB"]
...
def main():
    ...
    print(f"\nQPCA widen {tuple(args.widen)} x{args.widen_mult}; QPCA-EC uniform+ECSQ, "
          f"coder model frozen on calib; QPCA-EOB = same quantizer, EOB-coded rate")
    ...
    for b in k_bits:
        ...
        # EOB: identical quantizer (so identical distortion); only the coded rate differs
        eob_model = build_eob_model(delta, fetch_calib, root, args.calib_idx, b,
                                    n_layers, n_kv_heads, d_head)
        eob_rate[b] = coded_bits_eob_eval(fetch_eval, delta, eob_model, n_layers, n_kv_heads, d_head)
        print(f"  [EC]  b={b}: held-out rate = {ec_rate[b]:.3f} bits/coord")
        print(f"  [EOB] b={b}: held-out rate = {eob_rate[b]:.3f} bits/coord "
              f"(vs EC {ec_rate[b]:.3f}, saving {ec_rate[b]-eob_rate[b]:+.3f})")
    ...
    final["QPCA-EOB"] = final["QPCA-EC"]   # identical quantizer -> identical distortion; only rate differs
    ...
    for m in METHODS:
        for b in k_bits:
            dd = final[m][b]
            print(f"{m:<11} | {b:<2} | {rate_for(m, b):>9.3f} | {dd['top1']:>7.4f} | {dd['top5']:>7.4f} | "
                  f"{dd['k_mse']:>11.3e} | {dd['logit_err']:>11.3e}")
        print()
    print("rate(b/c): baselines = grid-bits; QPCA-EC = HELD-OUT constriction bits; "
          "QPCA-EOB = same, EOB-coded. Model frozen on calib, applied to eval. Calib/eval disjoint.")
    ...
    colors = {"TurboQuant":"#888","PCA":"#c1700a","JointQK":"#328ac1","QPCA":"#1c5d2c",
              "QPCA-EC":"#9b1c9b","QPCA-EOB":"#d62fb0"}
    markers = {"TurboQuant":"s","PCA":"D","JointQK":"o","QPCA":"^","QPCA-EC":"P","QPCA-EOB":"X"}
    def xfor(m): return [rate_for(m, b) for b in k_bits]
