...
1. **Qwen3-8B only.** Llama-3.1-8B has not been re-run at v7 setup. The `notes/stage1/v7_llama_runbook.md` walks the remote agent through the full pipeline; we don't yet have those numbers.
...
1. **Llama-3.1-8B v7 reproduction** on a remote machine using the runbook (`notes/stage1/v7_llama_runbook.md`). Cross-model agreement on the JointQK K=2 multi-doc QA win is the strongest cross-validation we can do.
2. **`--jobs-per-gpu 1` as v7 default** — update `experiments/stage1/scripts/launch_phase7_v7.sh` so the next sweep starts directly at the safe concurrency level. This Qwen run wasted ~9 GPU-hours on pass-1 thrashing.
...
- Raw results: `artifacts/stage1/downstream_v7/qwen3_8b/<config>_<task>/longbench__<task>__Qwen--Qwen3-8B__<press>__<ratio>/{metrics.json,predictions.csv,config.yaml}`. **Pass-1 successes write to canonical dir; pass-2 successes write to `<canonical>/N/` numbered subdirs.** The aggregator script in this report follows that convention.
- Per-cell logs: `experiments/stage1/logs/phase7_v7_qwen3_8b/job_*_a*.log`
- Dispatcher overview: `experiments/stage1/logs/phase7_v7_qwen3_8b/_overview.log` (pass 2; pass 1 was overwritten when pass 2 launched — pass-1 status was at-rest 40 OK + 152 OOM!)
- Pass-2 outer launcher log: `experiments/stage1/logs/phase7_v7_qwen3_8b_pass2_outer.log`
...
- v7 K calibration: `artifacts/stage1/cca_vs_waterfill_study/cca_stats_longbench_compact8_n400.pt` (sigma_q, sigma_k, R_sym pooled over 400 LongBench-compact8 train prompts)
- v7 V calibration: `artifacts/stage1/v_method_study/v_stats_longbench_compact8_n400.pt` (cov_v, mu_v — unused by v_turboquant but retained)
- v_lock.txt: `artifacts/stage1/v_method_study/v_lock.txt` (`V_METHOD=v_turboquant V_BITS=3 V_REL_F1_AT_LOCK=1.0080`)
- Eval exclude file: `artifacts/stage1/calibration_splits/longbench_compact8_60_seed20260504_2k32k/exclude_train_indices_for_eval.json`
- Disconnect investigation report (v6→v7 motivation): `notes/stage1/jointqk_disconnect_investigation.md`
- v6 prior report: `notes/stage1/phase7_v6_results_report.md`
- Llama remote runbook: `notes/stage1/v7_llama_runbook.md`
