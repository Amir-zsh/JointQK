...
This document is **self-contained** for an agent running the v7 sweep on Llama-3.1-8B-Instruct on a remote server. It assumes you already know the v7 design (otherwise read `notes/stage1/jointqk_disconnect_investigation.md` first); here we walk through commands.
...
The Llama run uses **distinct paths** at every stage so it cannot accidentally overwrite the existing Qwen3 artifacts at `artifacts/stage1/calibration/longbench_compact8_qkv/`:

| stage | Qwen3 path (existing, must not be touched) | Llama path (new, this runbook) |
|---|---|---|
| Capture run-root | `artifacts/stage1/calibration/longbench_compact8_qkv/` | `artifacts/stage1/calibration/longbench_compact8_qkv_llama31_8b/` |
| K calibration artifact | `artifacts/stage1/cca_vs_waterfill_study/cca_stats_longbench_compact8_n400.pt` | `artifacts/stage1/cca_vs_waterfill_study/cca_stats_llama31_8b_longbench_compact8_n400.pt` |
| V calibration artifact | `artifacts/stage1/v_method_study/v_stats_longbench_compact8_n400.pt` | `artifacts/stage1/v_method_study/v_stats_llama31_8b_longbench_compact8_n400.pt` |
| Downstream F1 | `artifacts/stage1/downstream_v7/qwen3_8b/` | `artifacts/stage1/downstream_v7/llama31_8b/` |
| Logs | `experiments/stage1/logs/phase7_v7_qwen3_8b/` | `experiments/stage1/logs/phase7_v7_llama31_8b/` |
...
3. **v7 launcher fail-loud.** `launch_phase7_v7.sh --model llama31_8b` checks for the Llama-suffixed calibration files at start. If they don't exist (because step 3 wasn't run yet), it prints the exact build command and exits before queuing any jobs.
...
- The repo synced to the remote machine. The local `sync_to_remote.sh` script does this:
  ...
  - `artifacts/stage1/v_method_study/v_lock.txt` (selects `v_turboquant`).
  - `artifacts/stage1/calibration_splits/longbench_compact8_60_seed20260504_2k32k/` (manifest, train/test row jsonls, exclude file — ~760 KB total). The capture stage reads `manifest.json`; the v7 launcher reads `exclude_train_indices_for_eval.json`.
...
```bash
.venv/bin/python -c "
from experiments.stage1.calibration.analyze_bases import combine_stats, jointqk_basis
...
.venv/bin/python experiments/stage1/calibration/launch.py \
...
.venv/bin/python experiments/stage1/scripts/phase7_worker.py \
    ...
    --log-dir experiments/stage1/logs/phase7_v7_smoke \
    --commands-file <(cat <<EOF
{"_label": "smoke_oracle_qasper", "press_name": "no_press", "compression_ratio": 0.0, "dataset": "longbench", "data_dir": "qasper", "fraction": 0.05, "output_dir": "artifacts/stage1/downstream_v7_smoke/llama31_8b/full_precision_qasper"}
{"_label": "smoke_jointqk_qasper", "press_name": "jointqk", "press_kwargs": {"cca_stats_path": "artifacts/stage1/cca_vs_waterfill_study/cca_stats_${SMOKE_SUFFIX}.pt", "v_stats_path": "artifacts/stage1/v_method_study/v_stats_${SMOKE_SUFFIX}.pt", "v_method": "v_turboquant", "k_bits": 2, "v_bits": 3, "compress_decode": false, "layer0_full_precision": true, "quantize_k": true, "quantize_v": true}, "dataset": "longbench", "data_dir": "qasper", "fraction": 0.05, "output_dir": "artifacts/stage1/downstream_v7_smoke/llama31_8b/jointqk_k2_v3_qasper"}
EOF
)

# 5. Verify outputs
ls artifacts/stage1/downstream_v7_smoke/llama31_8b/*/longbench__qasper*/metrics.json
cat artifacts/stage1/downstream_v7_smoke/llama31_8b/full_precision_qasper/longbench__qasper*/metrics.json
cat artifacts/stage1/downstream_v7_smoke/llama31_8b/jointqk_k2_v3_qasper/longbench__qasper*/metrics.json
```

**What success looks like:**
- Step 1: 16 capture entries (8 tasks × 2 prompts) finish in ~3 min on one GPU. Stats per shard written to `artifacts/stage1/calibration/<SMOKE_RUN_ID>/02_stats/`.
...
- The compressor cache dir doesn't exist → JointQK press auto-creates it; if you see permission errors, `mkdir -p artifacts/stage1/_compressor_cache && chmod u+w` it.

**Cleanup the smoke run** before launching real:
```bash
rm -rf artifacts/stage1/calibration/longbench_compact8_qkv_llama31_8b_smoke \
       artifacts/stage1/cca_vs_waterfill_study/cca_stats_llama31_8b_smoke.pt \
       artifacts/stage1/v_method_study/v_stats_llama31_8b_smoke.pt \
       artifacts/stage1/downstream_v7_smoke \
       experiments/stage1/logs/phase7_v7_smoke
```
...
```
artifacts/stage1/calibration_splits/longbench_compact8_60_seed20260504_2k32k/manifest.json
```
...
- Per-shard structured progress logs go to `experiments/stage1/logs/phase_capture/`. Watch progress with:
  ```bash
  tail -F artifacts/stage1/calibration/longbench_compact8_qkv_llama31_8b/logs/capture/_overview.log
  ```

**Disk cost:**
- Stats: ~75 MB × 480 = ~36 GB.
- Raw fp16 (test only): 1–10 GB per prompt × 80 ≈ ~500 GB.
- Total: budget ~600 GB free under `artifacts/stage1/calibration/longbench_compact8_qkv_llama31_8b/`.
...
.venv/bin/python -c "
import torch
agg = torch.load('artifacts/stage1/calibration/longbench_compact8_qkv_llama31_8b/02_stats/aggregate.pt', map_location='cpu', weights_only=False)
per = agg['per_example']
...
Outputs:
- `artifacts/stage1/cca_vs_waterfill_study/cca_stats_llama31_8b_longbench_compact8_n400.pt` (~90 MB)
- `artifacts/stage1/v_method_study/v_stats_llama31_8b_longbench_compact8_n400.pt` (~36 MB)
...
```bash
.venv/bin/python experiments/stage1/scripts/launch_phase7_v7.sh --model llama31_8b
```
...
- **Calibration train rows are excluded from the F1 evaluation** via `--exclude_indices_file` (the launcher passes this automatically). For the 7 calibration-overlapping tasks (hotpotqa, multi_news, musique, qasper, qmsum, repobench-p, triviaqa), 50/200 rows used to fit Σ_Q / Σ_K are dropped → eval N drops to ~150 on those tasks. The other 5 tasks (2wikimqa, lcc, narrativeqa, samsum, trec) are unaffected (not in the calibration corpus). Exclude file: `artifacts/stage1/calibration_splits/longbench_compact8_60_seed20260504_2k32k/exclude_train_indices_for_eval.json` — committed; no regeneration needed.
...
**Watch progress:**
```bash
tail -F experiments/stage1/logs/phase7_v7_llama31_8b/_overview.log
```
...
base = Path("artifacts/stage1/downstream_v7/llama31_8b")
...
Save the grid to `notes/stage1/phase7_v7_llama31_8b_results_report.md`. Compare to:
- The Qwen3-8B v7 numbers (when available) for cross-model agreement.
- The published v6 Qwen3-8B numbers in `notes/stage1/phase7_v6_results_report.md` to see how the calibration + V-method changes translate.
...
- **KIVI cells OOM at long context with 2 jobs/GPU.** v7's `--max-retries 10` should auto-requeue them and they'll usually fit once neighbour cells finish and free memory. If a KIVI cell shows up as `OOM!` (final OOM after all retries) in `_overview.log`, rerun just the failed cells at 1/GPU:
  ```bash
  bash experiments/stage1/scripts/launch_phase7_v7.sh --model llama31_8b --jobs-per-gpu 1
  ```
...
| purpose | path |
|---|---|
| Split manifest (immutable) | `artifacts/stage1/calibration_splits/longbench_compact8_60_seed20260504_2k32k/manifest.json` |
| Per-example raw fp16 (test-split only) | `artifacts/stage1/calibration/longbench_compact8_qkv_llama31_8b/01_raw/shard_NNN/*.pt` |
| Per-example stats | `artifacts/stage1/calibration/longbench_compact8_qkv_llama31_8b/02_stats/shard_NNN/*.pt` |
| Aggregated stats | `artifacts/stage1/calibration/longbench_compact8_qkv_llama31_8b/02_stats/aggregate.pt` |
| v7 K calibration artifact | `artifacts/stage1/cca_vs_waterfill_study/cca_stats_llama31_8b_longbench_compact8_n400.pt` |
| v7 V calibration artifact | `artifacts/stage1/v_method_study/v_stats_llama31_8b_longbench_compact8_n400.pt` |
| `v_lock.txt` (V_METHOD=v_turboquant) | `artifacts/stage1/v_method_study/v_lock.txt` |
| v7 launcher | `experiments/stage1/scripts/launch_phase7_v7.sh` |
| Downstream F1 outputs | `artifacts/stage1/downstream_v7/llama31_8b/<config>_<task>/.../metrics.json` |
| Per-cell logs | `experiments/stage1/logs/phase7_v7_llama31_8b/job_*_a0.log` |
| Compressor disk cache (built lazily) | `artifacts/stage1/_compressor_cache/*.pt` |
...
nohup bash experiments/stage1/scripts/launch_phase7_v7.sh \
    --model llama31_8b --gpus "$GPUS" \
    > experiments/stage1/logs/phase7_v7_llama31_8b_outer.log 2>&1 &
