...
LAUNCHER = REPO / "experiments/bench/parallel_launcher.py"
...
def write_commands(args: argparse.Namespace, stage: str, n_shards: int) -> Path:
    ...
    script = {
        "capture": REPO / "experiments/calibration/capture_raw.py",
        "stats": REPO / "experiments/calibration/compute_stats.py",
        "analysis": REPO / "experiments/calibration/analyze_bases.py",
    }[stage]
    extra = ""
...
def run_stage(args: argparse.Namespace, stage: str, n_shards: int) -> None:
    ...
    if stage == "stats":
        run_cmd([str(PY), str(REPO / "experiments/calibration/compute_stats.py"), *flags], args.dry_run)
        run_cmd([str(PY), str(REPO / "experiments/calibration/validate_artifacts.py"), "--stage", "stats", *flags], args.dry_run)
    elif stage == "analysis":
        extra = ["--empirical" if args.empirical else "--no-empirical"]
        ...
        run_cmd([str(PY), str(REPO / "experiments/calibration/analyze_bases.py"), *flags, "--merge-only", *extra], args.dry_run)
        run_cmd([str(PY), str(REPO / "experiments/calibration/validate_artifacts.py"), "--stage", "analysis", *flags], args.dry_run)
    elif stage == "capture":
        run_cmd([str(PY), str(REPO / "experiments/calibration/validate_artifacts.py"), "--stage", "raw", *flags], args.dry_run)
    elif stage == "charts":
        run_cmd([str(PY), str(REPO / "experiments/calibration/make_charts.py"), "--artifact-root", str(args.artifact_root), "--run-id", args.run_id], args.dry_run)
        run_cmd([str(PY), str(REPO / "experiments/calibration/validate_artifacts.py"), "--stage", "charts", *flags], args.dry_run)
...
def main() -> None:
    ...
    run_cmd([str(PY), str(REPO / "experiments/calibration/validate_artifacts.py"), "--stage", "split", *base_flags], args.dry_run)
    for stage in stages:
