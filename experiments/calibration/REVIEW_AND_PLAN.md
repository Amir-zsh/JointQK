...
Pipeline lives in `experiments/calibration/`:
...
5. **Calibration's `allocate_bits` ≠ production `water_fill`.** The calibration's bisection + post-hoc cap_bits diverges from `experiments/toolkit/metric_transform.water_fill`'s iterative-saturation loop and from `build_jointqk_compressor`'s rounding+cap. Calibration winner ≠ deployed winner is possible.
