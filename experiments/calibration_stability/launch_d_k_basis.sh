...
#
# Usage:
#   bash experiments/calibration_stability/launch_d_k_basis.sh --gpus 6,7
#   tail -f experiments/logs/phase1d_k_basis_stability.log
...
GPUS="6"
BUNDLE="${REPO_ROOT}/artifacts/query_stats_longbench_under4k"
OUT="${REPO_ROOT}/artifacts/v_bases/k_basis_stability"
LOG="${REPO_ROOT}/experiments/logs/phase1d_k_basis_stability.log"
...
for shard_id in "${!GPU_ARR[@]}"; do
    ...
    cmd="CUDA_VISIBLE_DEVICES=${gpu} ${REPO_ROOT}/.venv/bin/python ${REPO_ROOT}/experiments/calibration_stability/run_d_k_basis.py \
        --device cuda \
        --bundle ${BUNDLE} \
        --output-dir ${OUT} \
...
"${REPO_ROOT}/.venv/bin/python" "${REPO_ROOT}/experiments/calibration_stability/aggregate_d_k_basis.py" \
    --rows "${rows_files[@]}" \
