...
Each GPU is owned by exactly one worker process, so a worker only ever runs one
job at a time on its GPU. Jobs are picked from a FIFO queue as workers finish.
This avoids the round-robin collision where job index N+k could land on a GPU
still busy with job k.
...
def gpu_worker(gpu, in_queue, out_queue, log_dir):
    ...
    while True:
        ...
        log_file = log_dir / f"job_{job_idx:03d}_gpu{gpu}_{label}.log"
        env = os.environ.copy()
        env["CUDA_VISIBLE_DEVICES"] = str(gpu)
        start = time.time()
        with log_file.open("w") as f:
            f.write(f"# job {job_idx} | GPU {gpu} | label {label} | start {time.ctime(start)}\n")
            f.write(f"# cmd: {cmd}\n\n")
            f.flush()
            rc = subprocess.run(["bash", "-c", cmd], env=env, stdout=f, stderr=subprocess.STDOUT).returncode
        out_queue.put((job_idx, gpu, rc, time.time() - start, str(log_file)))
...
def main():
    ...
    overview.write_text(f"# {len(cmds)} jobs across GPUs {gpus} | start {time.ctime()}\n\n")
    ...
    workers = []
    for gpu in gpus:
        w = mp.Process(target=gpu_worker, args=(gpu, in_q, out_q, str(log_dir)))
        w.start()
        workers.append(w)

    fail = 0
    for _ in range(len(cmds)):
        job_idx, gpu, rc, elapsed, log_file = out_q.get()
        status = "OK  " if rc == 0 else "FAIL"
        line = f"[{time.strftime('%H:%M:%S')}] job {job_idx:03d} GPU {gpu} {status} {elapsed:6.0f}s {Path(log_file).name}"
