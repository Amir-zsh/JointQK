...
LOG = REPO / "experiments/stage1/logs/phase7_chain.log"
...
def main():
    ...
    while True:
        n = count_oks(REPO / "experiments/stage1/logs/phase6/_overview.log")
    ...
    decision_file = REPO / "artifacts/stage1/downstream/qwen3_8b/decode_scope/decode_decision.txt"
    if decision_file.exists():
        ...
        subprocess.run([
            ...
            "python -u -m experiments.stage1.eval.aggregate_decode_scope "
            "--input-dir artifacts/stage1/downstream/qwen3_8b/decode_scope "
            "--output-decision artifacts/stage1/downstream/qwen3_8b/decode_scope/decode_decision.txt"
        ], cwd=REPO)
    ...
    for cmd in [
        "python -u -m experiments.stage1.eval.aggregate_longbench "
        "--qwen-dir artifacts/stage1/downstream/qwen3_8b "
        "--llama-dir artifacts/stage1/downstream/llama31_8b "
        "--output artifacts/stage1/downstream/longbench_summary.json",
        "python -u -m experiments.stage1.eval.aggregate_ruler "
        "--qwen-dir artifacts/stage1/downstream/qwen3_8b "
        "--llama-dir artifacts/stage1/downstream/llama31_8b "
        "--output artifacts/stage1/downstream/ruler_summary.json",
    ]:
        full = (
            "source ~/miniconda3/etc/profile.d/conda.sh && conda activate efficient-llm && "
            + cmd
