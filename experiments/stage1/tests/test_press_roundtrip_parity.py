...
def main():
    ...
    bundle = Path("artifacts/stage1/query_stats_longbench_under4k/examples/ex_000.pt")
    b = torch.load(bundle, map_location="cpu", weights_only=False)
    k = b["k_post"][LAYER, HEAD].float()  # (S, D)
    v = b["v"][LAYER, HEAD].float()

    cca_path = "artifacts/stage1/cca_vs_waterfill_study/cca_stats.pt"
    cca = torch.load(cca_path, map_location="cpu", weights_only=False)
    v_path = "artifacts/stage1/v_method_study/v_stats.pt"
