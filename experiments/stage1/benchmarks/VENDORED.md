...
- **`evaluate_registry.py`** — trimmed copy of `kvpress/evaluation/evaluate_registry.py`. Changes:
  ...
  - Scorer import paths rewritten from `benchmarks.<name>.calculate_metrics` (flat `sys.path`-based imports used by upstream's `evaluate.py`) to `experiments.stage1.benchmarks.<name>.calculate_metrics` (absolute imports now that this tree is a Python package).
...
`experiments/stage1/evaluate_generations.py` uses this registry directly.

A byte-equal-to-upstream regression test (`experiments/stage1/tests/test_evaluation.py::test_vendored_longbench_matches_upstream_kvpress`) guards against silent drift if this directory is re-vendored.

## Re-vendoring from upstream

```bash
# From the repo root, assuming ../kvpress is the upstream checkout.
rm -rf experiments/stage1/benchmarks/{aime25,infinite_bench,longbench,longbenchv2,loogle,math500,needle_in_haystack,ruler,zero_scrolls}
cp -r kvpress/evaluation/benchmarks/* experiments/stage1/benchmarks/
