...
#
# See `notes/jointqk_disconnect_investigation.md` for the analysis driving
# these v7 changes.
...
if [[ ! -f "$CCA" ]]; then
    ...
    echo "Build it with: .venv/bin/python experiments/scripts/build_calibration_artifacts_from_pool.py --run-id longbench_compact8_qkv_${MODEL_TAG} --output-suffix ${CCA_SUFFIX}" >&2
    exit 1
fi
...
if [[ ! -f "$EXCLUDE_INDICES_FILE" ]]; then
    ...
    echo "Generate it with: .venv/bin/python -c \"import json; from pathlib import Path; m=json.loads(Path('artifacts/calibration_splits/longbench_compact8_60_seed20260504_2k32k/manifest.json').read_text()); excl={}; [excl.setdefault(r['config'],[]).append(int(r['row_index'])) for r in m['rows'] if r['split']=='train']; Path('$EXCLUDE_INDICES_FILE').write_text(json.dumps(excl, indent=2))\"" >&2
    exit 1
fi

OUT_BASE="${REPO_ROOT}/artifacts/bench/${MODEL_TAG}"
LOG_DIR="${REPO_ROOT}/experiments/logs/phase7_v7_${MODEL_TAG}"
...
${VENV_PYTHON} experiments/bench/worker.py \
    --model "$MODEL" \
    --commands-file "$CMDS" \
    --log-dir "$LOG_DIR" \
    --gpus "$GPUS" \
