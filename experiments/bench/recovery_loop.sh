...
MODEL_TAG=qwen3_8b
GPUS=0,1,2,3,4,5
OUT_BASE="$REPO/artifacts/bench/$MODEL_TAG"
LOG_DIR="$REPO/experiments/logs/phase7_v7_$MODEL_TAG"
...
for cfg in "1 10" "1 5"; do
    ...
    bash experiments/bench/launch.sh \
        --model "$MODEL_TAG" \
        --gpus "$GPUS" \
        --jobs-per-gpu "$JPG" \
        --max-retries "$RETRIES" \
...
out_base = Path("artifacts/bench/qwen3_8b")
cmds = Path("experiments/logs/phase7_v7_qwen3_8b/commands.jsonl").read_text().splitlines()
missing = []
for line in cmds:
