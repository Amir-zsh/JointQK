...
LOG = REPO / "experiments/logs/phase7_chain.log"
...
def main():
    ...
    while True:
        n = count_oks(REPO / "experiments/logs/phase6/_overview.log")
    ...
    if decision_file.exists():
        ...
        subprocess.run([
            ...
            "python -u -m experiments.eval.aggregate_decode_scope "
    ...
    subprocess.run([
        "bash", str(REPO / "experiments/bench/launch_longbench.sh"),
        "--gpus", "0,1,2,3,4,5",
        "--model", "qwen3_8b",
        "--fraction", "0.5",
    ], cwd=REPO)
    ...
    subprocess.run([
        "bash", str(REPO / "experiments/bench/launch_ruler.sh"),
        "--gpus", "0,1,2,3,4,5",
        "--ks", "4",
    ...
    for cmd in [
        "python -u -m experiments.eval.aggregate_longbench "
        ...
        "python -u -m experiments.eval.aggregate_ruler "
        "--qwen-dir artifacts/downstream/qwen3_8b "
        "--llama-dir artifacts/downstream/llama31_8b "
